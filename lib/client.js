...
exports.defs = stream.defs;
exports.time = stream.time;
...
function Client(opts) {
	...
	this.getPlayer = function (cn) {
		...
		if (!this.players[cn]) this.players[cn] = { "suicides": 0, "teamkills": 0, "accuracy": 0, "deaths": 0, "ip": 0, "country": "" };
	...
	this.pingRemote = function () {
		...
		if (opts.stayInSpec && self.client.state != defs.playerStatesN.CS_SPECTATOR) self.sendf(1, "riii", defs.MSG_TYPES_N.N_SPECTATOR, self.client.clientnum, 1);
		
		if (this.timeLeft <= 0) this.emit("intermission");
	}
	...
	this.on("packet.N_DAMAGE", function (st) {
		...
		var damage = this.getPlayer(st.getInt());
		var armour = this.getPlayer(st.getInt());
		var health = this.getPlayer(st.getInt());
    ...
    var player = this.getPlayer(st.getInt());
		...
		if (flag >= this.remote.flags.length) return;
	...
	enet.createClient({peers: 2, channels: 3, down: 0, up: 0}, function(err, host) {
		...
		host.on("connect", function (peer, data) {
			...
			peer.on("disconnect", function (packet, channel) {
				...
				if (this.extinfoHandle) clearInterval(this.extinfoHandle);
